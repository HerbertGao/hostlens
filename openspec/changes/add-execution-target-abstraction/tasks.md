...
- [ ] 5.5 实现 SSH 连接超时（`asyncssh.connect(connect_timeout=...)`）+ raise `TargetError(kind="ssh_connect_timeout", target=self.name)`（含 host:port 进 extra 字段）；asyncssh 配置 `agent_forwarding=False, x11_forwarding=False, keepalive_interval=60`；验收：单测覆盖错误 kind 与脱敏（不含凭据）
