# runtime-inspector-suite 规范

## 目的

定义**语言运行时** inspector 套件契约——以独立 capability 承接「目标运行时进程/端点在场」这一与 os-shell（零外部服务依赖）/ service（单实例服务 + 连接 secret remap）两套件正交的前提，按 `TODO.md` §M6 覆盖矩阵覆盖此前空白的 JVM 与 Go 运行时域。套件内每个 inspector 为遵守作者契约的纯 YAML、参数化目标运行时进程或端点且参数注入安全、目标不在场时 fail-loud 而非伪造健康、每个附 ReplayTarget fixture 与可证检出的 snapshot 测试、本套件禁止引入新基础设施。
