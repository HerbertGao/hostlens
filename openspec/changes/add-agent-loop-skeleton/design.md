...
- `ToolsAdapter.list_for_agent() -> list[dict[str,Any]]`；**`ToolsAdapter.dispatch(name, args_json: dict, ctx=None) -> dict[str,Any]`**（实证：返回 `result.model_dump()` dict，**不是** BaseModel）。dispatch 真实错误语义：① 步骤5 input schema 校验失败 raise `TypeError`；② handler 内部异常被 dispatch **捕获并 scrub** 成 `{"is_error": True, "error_kind", "tool_name", "message", "cause"}` envelope dict **返回**（不 raise）；③ `ToolPolicyViolation` / `KeyError` / `asyncio.CancelledError` **不 wrap，原样 raise**；④ `spec.timeout` 非空时 dispatch 自带 `asyncio.wait_for` 超时；⑤ dispatch 内部已用 `scrub_exception_message`（循环对 ② 的 envelope 不可二次 scrub）。
...
### D-2：`cache_control` 只在 `system` 上、且只由 loop 注入

循环持有两份 system 表示由调用方传入？不 —— M2.2 调用方只传一个 `system`（`list[dict]` 或 `str`）。loop 内部：当 `capabilities.prompt_caching == True` 且 system 是 `list[dict]` 形态时，在**最后一个** system block 上加 `{"cache_control": {"type": "ephemeral"}}`；为 False 时原样透传。

理由：骨架不该决定「缓存哪些内容」（那是 M2.5 策略），只该保证「机制开关由 capability 驱动且对 backend 诚实」。backend 端 `check_capability_consistency` 是兜底；loop 端是第一道闸。

**替代：** 由调用方传两份（plain / cached）system → 推迟到 M2.4（届时 Planner 拥有 system prompt 全文，按 §4.8 决定缓存边界）。M2.2 先做单 system + loop 注入，足够单测验证 gate 行为。
...
`gather` 不用 `return_exceptions=True`：路径 1–3 与幻觉拦截的 `_dispatch_one` 自己产出 tool_result dict 永不抛；路径 4–6 必须中断整轮，让其自然从 gather 传播即可（gather 默认首个异常即 propagate）。每个 tool_result 用 `block.id` 作 `tool_use_id` 一一对应。
