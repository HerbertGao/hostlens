...
- **M7-ext 读期（已落地）**：在只读三件套之上追加 **7 个只读管控工具**（`add-mcp-readonly-management-tools`），把 MCP 暴露的工具集从 **3 → 10**：`list_schedules` / `get_schedule_status` / `run_schedule_now` / `list_channels` / `list_reports` / `show_report` / `diff_reports`，让远程 LLM 在一次对话里完成「看调度 → 跑整包巡检 → 读报告 → 跑 regression diff」闭环。全部 `surfaces={"agent","mcp"}` + `side_effects ∈ {none, read}` + `requires_approval=False`，天然过既有 `dispatch` 门——**零 adapter/server/gate 改动**，兑现 M7 架构「加只读工具 = 只加 ToolSpec」。依赖经注册期闭包注入（`register_mcp_management_tools(registry, deps=...)`，`ToolContext` 6 字段冻结集不动，ADR-008）；`list_channels` 仅暴露 name/type（绝不返回 token/secret）；`run_schedule_now` 复用 scheduler runner 但以 `dispatch_notify=False` 抑制通知、消耗 token 非免费；`show_report` / `diff_reports` 的查询键是 report-store `report_id`（≠ scheduler ledger `run_id`）。工具清单与各 `mcp_description` 见 [docs/integrations/mcp-tools.md](integrations/mcp-tools.md)。写工具（加 target / 发通知 / 删 target）属独立写期提案，不在本期
