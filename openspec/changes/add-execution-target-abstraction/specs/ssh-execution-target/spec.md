...
- **重连流程仅适用于"已建立连接断开"场景**（已经 SSH 握手成功后中途断开），**禁止**用于首次 connect。两条路径必须严格分开：
  - **首次 connect**（lazy 建立 `self._conn`）：`asyncssh.connect(..., connect_timeout=entry.connect_timeout or 10)`（**只**用 `entry.connect_timeout` per-target override 或硬默认 10s；M1 范围内**不**在 `Settings.ssh` 加 `connect_timeout` 字段，避免 task 4.3a 工作量蔓延；M6+ 视实际运维场景再决定是否提到 Settings 层）；按异常类型分类 raise：
    ...
    - `asyncssh.PermissionDenied` / `asyncssh.BadHostKeyError` / `asyncssh.misc.KeyExchangeFailed`（认证 / host key 不匹配 / KEX 失败） → `TargetError(kind="ssh_auth_failed", target=self.name)` + **三层 password scrub**（见 §需求:SSH 凭据... §场景:password 不出现在 SSH 连接失败的错误日志（三层脱敏））
