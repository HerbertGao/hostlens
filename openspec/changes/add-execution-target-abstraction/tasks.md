...
- [ ] 2.4 实现 `hostlens.core.exceptions.TargetError` 子类（继承自 M0 已落地的 `HostlensError`），签名为 `TargetError(kind: str, *, target: str | None = None, original: Exception | None = None, **extra)` —— **首个位置参数是 `kind`**，`target` 字段统一表示**该 error 涉及的 target identifier**（无论是"找不到 target"还是"在 target 上做某事失败"，都用同一字段名，避免 `name=` vs `target=` 双字段名引起调用方误用）；更新 `hostlens.core.exceptions.__all__` 公共导出（M0 测试需同步 expected count）；验收：`tests/core/test_exceptions.py` 公共导出断言更新 + 覆盖 `TargetError(kind="x", target="y")` 与 `TargetError("ssh_auth_failed", target=name)` 两种调用风格；grep 仓库**禁止**出现 `TargetError(... name=...)` 调用（统一用 target）
...
- [ ] 5.3 实现 SSH 断线重连：按 ssh-execution-target spec §需求 中给出的精确算法（`for delay in [1.0, 4.0, 16.0]: sleep(delay); try connect; except continue`），最多 3 次 connect 尝试 + 3 段 sleep；穷尽则 raise `TargetError(kind="ssh_connection_lost", target=name)`；验收：(a) 单测 mock `asyncssh.connect` 第 1 次 attempt 抛 ConnectionLost、第 2 次成功（即首次失败 + 1s sleep + 2nd attempt 成功），断言 exec 最终成功 + 用了 2 次 connect 调用 + sleep(1.0) 被调用 1 次；(b) **完整退避序列单测**（穷尽路径）：mock connect 永远抛 ConnectionLost，断言 sleep 依次被调用为 `1.0` → `4.0` → `16.0`（3 次）+ connect 共 3 次调用 + 最终 raise `TargetError(kind="ssh_connection_lost")`
