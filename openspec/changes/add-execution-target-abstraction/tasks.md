...
- [ ] 2.4 实现 `hostlens.core.exceptions.TargetError` 子类（继承自 M0 已落地的 `HostlensError`），签名为 `TargetError(kind: str, *, target: str | None = None, original: Exception | None = None, **extra)` —— **首个位置参数是 `kind`**（matches `TargetError("ssh_auth_failed", target=name)` 调用风格，与 proposal / spec 一致），其余 keyword-only；更新 `hostlens.core.exceptions.__all__` 公共导出（M0 测试需同步 expected count）；验收：`tests/core/test_exceptions.py` 公共导出断言更新 + 覆盖 `TargetError(kind="x", target="y")` 与 `TargetError("ssh_auth_failed", target=name)` 两种调用风格
...
- [ ] 4.1 实现 `hostlens.targets.registry.TargetRegistry`（API：`register(target, entry)` / `get(name)` / `get_entry(name)` / `names()` / `list()` / `list_entries()`；name 冲突 raise）；**`register` 在添加前必须用 `re.fullmatch(r"^[a-z][a-z0-9_\-]{0,63}$", target.name)` 校验 name，不匹配 raise `TargetError(kind="invalid_target_name", name=target.name)`**（作为绕过 loader 的最后一道防线）；同时持有 `ExecutionTarget` 实例与 `TargetEntry` 元数据双索引；验收：单测覆盖 spec 全部 5 个场景（含 get_entry / list_entries + register 拒绝非法 name）
- [ ] 4.1a **LocalTarget / SSHTarget 构造器 name 校验**：`__init__` 在赋值 `self.name` 前 `re.fullmatch(...)`，不匹配 raise `TargetError(kind="invalid_target_name", name=name)`；验收：单测构造 `LocalTarget(name="Prod-Web")` / `LocalTarget(name="1web")` / `SSHTarget(name="prod web")` 都 raise；构造 `LocalTarget(name="prod-web")` 成功
...
- [ ] 4.3a **扩展 M0 `hostlens.core.config.Settings`**：增加 `targets_config_path: Path = Path("~/.config/hostlens/targets.yaml").expanduser()` 字段（pydantic-settings 风格），支持 `HOSTLENS_TARGETS_CONFIG_PATH` env var override；CLI / doctor / build_registry_from_config 都从 `Settings.targets_config_path` 读路径，**禁止** hardcode `~/.config/hostlens/targets.yaml` 字面量；验收：(a) 单测覆盖默认值 + env var override；(b) `grep -rn "~/.config/hostlens/targets.yaml" src/hostlens/cli src/hostlens/targets` 仅 Settings 字段定义处出现，不在调用方出现
...
- [ ] 5.3 实现 SSH 断线重连（捕获 `ConnectionLost` / `ChannelOpenError` → **1 次自动重连**，退避序列 1s → 4s → 16s；严格对齐 docs/OPERABILITY.md §2.2「连接中断 → 1 次自动重连（指数退避 1s→4s→16s）」；解读：1 次重连 = 1 次新的 `asyncssh.connect()` 尝试，重连**前**的等待序列 1s/4s/16s 是按顺序的退避尝试组——具体说每次 connect 失败后 sleep 下一个序列值再尝试；序列耗尽（最后 16s 后仍失败）则 raise `TargetError(kind="ssh_connection_lost", target=name)`）；验收：(a) 单测 mock `asyncssh.connect` 第 1 次抛 ConnectionLost、第 2 次成功，断言 exec 最终成功 + 用了 2 次 connect 调用（首次 + 1 次重连）；(b) **退避序列单测**：用 `freezegun` 或 mock `asyncio.sleep` 计数，断言 connect 失败序列中 sleep 被调用且参数依次为 `1.0` → `4.0` → `16.0`（验证生产真按 OPERABILITY 退避，不是单测通过但生产跳过 sleep）
