...
- **服务器批量纳管（Target Onboarding）** —— M7-ext 读期管控工具集已落（#101）；原 M7-ext 写期 `add-mcp-write-approval-flow`（`add_target`/`remove_target` 等走两段式 MCP approval gate）**已废弃**——经 `/opsx:explore` + Codex/架构 review + Gate 0 实测（asyncssh 直连 Tailscale SSH 6/6 cred-less 通过）收敛为「批量纳管」路线，拆为 `add-cli-target-import`（主，CLI 批量导入 + 探活 + 原子幂等写）+ `add-mcp-target-import-propose`（次，MCP 只 propose 产 ImportPlan 不落盘 ≡ M9 runbook 模式）。`remove_target` 不上 MCP；`test_channel`/`notify_report` 各自独立小提案。详见 [docs/roadmap/target-onboarding.md](docs/roadmap/target-onboarding.md)
