...
- **服务器批量纳管（Target Onboarding）✅ 主线已落** —— M7-ext 读期管控工具集已落（#101）；原 M7-ext 写期 `add-mcp-write-approval-flow`（两段式 MCP approval gate）**已废弃**，收敛为「批量纳管」路线。`add-cli-target-import`（主，CLI 批量导入 + 探活 + 原子幂等写）已 merged+archived（#102）；`add-mcp-target-import-propose`（次，MCP 只 propose 产 `ImportPlan` 不落盘 ≡ M9 runbook 模式；CLI `target import --from-plan` 本地落地）已实现（待 PR）。`remove_target` 不上 MCP；`test_channel`/`notify_report` 各自独立小提案（未起）。详见 [docs/roadmap/target-onboarding.md](docs/roadmap/target-onboarding.md)
