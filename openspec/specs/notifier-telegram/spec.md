...
- **失败检查**(本提案新增):覆盖行的 `{failed}` 计数**仅是数字**,运维无从判断是哪台机 / 为什么失败(真故障 vs 连通性抖动)。故当 `failed > 0` 时**必须**渲染「失败检查」段,筛 `report.inspector_results` 里 `status ∈ _FAILED_STATUSES`(`{timeout, target_unreachable, exception}`,**复用** `coverage_line` 既有 frozenset、**不**用 `status not in {ok, requires_unmet}` 负谓词,对齐闭集纪律防未来第 6 个 status 静默归 failed)的项,**按 `(target_name, 原因 label)` 分组**:每组一行主机名 + **中文原因 label** + 该组失败的 inspector 名单。**原因 label 必须先 key on `status`**(5 值闭枚举,恒可映射):`timeout`→执行超时 / `target_unreachable`→不可达 / `exception`→采集异常;**仅当 `status == "target_unreachable"`**(此时 `error == TargetError.kind`,是枚举串)再以 `error` 细化:`ssh_connect_timeout`→连接超时 / `ssh_auth_failed`→认证失败 / `ssh_connect_failed`→连接失败 / `ssh_connection_lost`→连接中断,**未知 kind**(`ssh_no_entry` / `target_disabled` / docker·k8s kinds…)→回退桶级中文「不可达」。**禁止以 `error` 为 `timeout`/`exception` 的键**:这两类 status 的 `error` 是自由文本句(`"collect.command exceeded 30 seconds"` / `"parse_failed: …"`,且模型校验器强制非空),以 `error` 为键会①渲染生英文②同主机多个 `exception` 因 message 不同被拆成 N 组(破坏「合一组防刷屏」)。`failed == 0` 时**禁止**渲染该段。消费**已脱敏**报告(`_redact_inspector_result` 保留 `status`/`target_name`/`error`),零模型字段改动。**该段 meta-independent**:数据源 `Report.inspector_results`(`min_length=1`,恒在、与 `report.meta` 无关),故 `meta is None` 时覆盖行省略但失败检查段仍渲染(紧随抬头;Telegram 纯 markdown 无 JSON 逗号脆弱性,只需空行/段距正确)。**`error` 脱敏稳定**:`target_unreachable` 的 `error == TargetError.kind`(枚举串,`redact_text` no-op,字节不变),`_FAIL_KIND_LABELS` 在已脱敏报告上查表命中。
- **段顺序「失败检查 + 发现优先」**:段顺序**必须**为 抬头 → 覆盖行 → **失败检查** → **发现** → **根因分析**。即「失败检查」段(仅 `failed > 0`)**必须**渲染在「发现」段之前(直接展开覆盖行的失败计数、且「没跑成的检查」是运维需先知道的覆盖缺口),「发现」段**必须**渲染在「根因分析」段之前(运维先看客观事实、再看 LLM 推测)。抬头 / 覆盖行 的位置与内容**不变**。**健康态(`✅ 未发现异常`)是「发现」段的空态替代,占据「发现」段的位置**——故无 finding 但有失败检查时,顺序为 抬头 → 覆盖行 → 失败检查 → 健康态(→ 根因分析,若有 hypotheses)。
...
既有 MarkdownV2 转义、`validate_config`、`sendMessage` 发送、宿主机本地时区渲染需求**不变**。「失败检查」段的主机名 / 原因 / inspector 名一律经 `mdv2_escape`。
...
#### 场景:有失败检查时按目标与原因分组列出（reason 先 key on status，本提案新增）
- **当** 一个 fleet 报告里 `inspector_results` 含:`cloudcone` 的 3 个 inspector `status=target_unreachable` / `error="ssh_connect_timeout"`;`vultr` 的 2 个 inspector `status=exception`、但 `error` 各为 `"parse_failed: …"` 与 `"output_schema_mismatch: …"`(**自由文本、互不相同**)
- **那么** **必须**渲染「失败检查」段:`cloudcone · 连接超时`(`target_unreachable` 经 `error` kind 细化)+ 其 3 个 inspector 名单;`vultr · 采集异常`(`exception` **key on status**,**不**读自由文本 `error`)+ 其 **2 个** inspector 名单**合一组**(尽管两条 `error` 文本不同——证明分组键是 status-derived label 而非 `error`);主机名 / label 经 `mdv2_escape`。**禁止**渲染 `vultr · parse_failed: …` 生英文、**禁止**把 vultr 两条拆成两组

#### 场景:无失败时不渲染失败检查段（本提案新增）
- **当** report 的 `inspector_results` 全部 `ok` 与 `requires_unmet`(`failed == 0`)
- **那么** **禁止**渲染「失败检查」段(无段头、无空段)

#### 场景:失败检查段在覆盖行之后、发现/健康态之前（本提案新增）
- **当** 一个 `failed > 0` 且无 finding(健康态)的报告(如今日 cloudcone 连不上但其余机器无异常)
- **那么** 段顺序**必须**为 抬头 → 覆盖行 → `失败检查` → `✅ 未发现异常`;「失败检查」段**必须**在健康态之前
...
#### 场景:单台 finding 的 fleet 报告仍按主机标注
