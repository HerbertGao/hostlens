...
- [ ] 4.3 实现 `hostlens.targets.config.load_targets_config(path) -> TargetsConfig` —— 含 `${ENV_VAR}` 展开 / 占位仅 `password`/`passphrase` 字段允许 / **文件不存在返回空 `TargetsConfig(version="1", targets=[])`**（**不**是 TargetRegistry —— 装配 registry 由 4.4 `build_registry_from_config` 负责）+ INFO log；同时 `name` 字段必须 enforce `^[a-z][a-z0-9_\-]{0,63}$` 正则；ConfigError 调用走 task 2.5 扩展后的 `(kind=..., var_name=..., target=...)` 签名；验收：单测覆盖 spec §需求:`TargetsConfig` 必须从 yaml 加载且环境变量占位展开 全部场景 + name 正则违反 raise + ConfigError kind 字段被正确填充
...
- [ ] 5.3 实现 SSH 断线重连（捕获 `ConnectionLost` / `ChannelOpenError` → **1 次自动重连**，退避序列 1s → 4s → 16s；严格对齐 docs/OPERABILITY.md §2.2「连接中断 → 1 次自动重连（指数退避 1s→4s→16s）」；超出 raise `TargetError(kind="ssh_connection_lost", target=name)`）；验收：单测 mock asyncssh.connect 第 1 次抛 ConnectionLost、第 2 次成功，断言 exec 最终成功 + 用了 2 次 connect 调用（首次 + 1 次重连）
