...
> 🚧 **开发中（M0–M5、M7、M8 已落地，M6 进行中，未发 PyPI）** —— `src/` 已有可运行代码，`hostlens` CLI 可装可跑：**M0** 脚手架 + doctor、**M1** ExecutionTarget / Inspector / Report + `hostlens inspect`、**M2** 手写 Agent loop（自然语言意图 → 自选 Inspector → markdown 报告）、**M3** Diagnostician + 根因假设 + regression diff、**M4** Scheduler（cron/interval 定时巡检 + Run 留痕 + daemon 优雅停机 + `hostlens schedule`）、**M5** Notifier（Telegram MarkdownV2 + 飞书 Lark HMAC 签名 + `only_if` 路由 + Scheduler↔Notifier 接线 + `hostlens notify`）、**M6**（🚧 进行中）内置 Inspector 库扩充（`builtin/` 下 65 个 inspector，覆盖 Linux/网络/Nginx/MySQL/PostgreSQL/Redis/Docker/安全基线/包管理/TLS/JVM·Go 运行时/日志等域）、**M7** MCP Server（stdio + `hostlens mcp serve`，暴露只读巡检三件套给远程 LLM）、**M8** Docker / Kubernetes ExecutionTarget（只读 exec 进容器 / Pod 巡检，容器安全的 28 个 inspector 直接可用）。尚未发布 PyPI 包（本地 `pip install -e` 安装）。
>
> 下面的「受控修复」（M9）等章节展示的是 **目标 UX 与规划中的命令形态** —— 这些里程碑尚未实现。已实现并可直接用的命令：`inspect` / `demo` / `schedule` / `notify` / `doctor` / `reports` / `mcp`（Telegram / 飞书 **通知推送**随 M5 落地：`schedule` 的 `notify` 路由会在 Report 持久化后实际派发；**MCP Server** 随 M7 落地：`hostlens mcp serve` stdio 暴露 `list_inspectors` / `list_targets` / `run_inspector`）。
>
...
- 🪄 **CLI + MCP 双交付** — 既是命令行工具（`hostlens inspect ...`），又是 MCP Server（M7 已落地：`hostlens mcp serve` stdio，让 Claude Desktop / Cursor 等 MCP host 把它当工具用）
...
stdio 前台 MCP Server，暴露只读巡检三件套（`list_inspectors` / `list_targets` / `run_inspector`）给远程 LLM。`mcp` 为 optional dependency（`pip install "hostlens[mcp]"`）；HTTP transport 不在本期范围。

**Demo Path**（无需 SSH / 无需付费 API）：

```bash
pip install -e ".[mcp]"           # 安装含 mcp SDK 的可选依赖
hostlens doctor --json            # 确认 checks.mcp 为 ok（缺失时为 missing，非致命）
hostlens mcp serve                # stdio 前台启动，由 MCP host 拉起
```

接入 Claude Desktop（macOS：`~/Library/Application Support/Claude/claude_desktop_config.json`）：
...
> `mcp` SDK 未安装时 `hostlens mcp serve` 退出码 1 并提示 `pip install "hostlens[mcp]"`；`sensitive_output` 未显式声明的工具不会出现在 MCP tool 列表（fail-closed 三处对称：投影 / `build_server` eager / dispatch）。
...
- [x] Inspector 插件加载器与注册中心
- [x] ExecutionTarget 抽象 + SSH/Local 实现
- [x] Planner Agent（基于 Anthropic SDK 的手写 tool-use loop）
- [ ] 内置 Inspector 集（Linux 系统、Nginx、MySQL、Redis、Docker 已覆盖；K8s 域 inspector 待独立提案）
- [x] Diagnostician Agent + 关联规则
- [x] CLI（target / inspect / inspectors / doctor / schedule / notify / mcp）
- [x] MCP Server（stdio；HTTP transport 后续 follow-up）
- [x] Docker / Kubernetes ExecutionTarget
- [x] Scheduler（APScheduler cron + interval + daemon）
- [x] Notifier 抽象 + 内置 Telegram / 飞书 Lark 适配器
- [ ] 报告渲染（Markdown / JSON / 飞书富文本卡片 / Telegram MarkdownV2 已落地；HTML 待做）
- [x] Regression diff：与上次巡检报告的对比
