...
### 5.3 PR 必须先过对抗性 review（AI 协作者强制）

任何由 AI 协作者（Claude Code / Codex / Cursor 等）完成的 commit-ready 工作（代码 / OpenSpec 提案 / 设计文档 / 配置变更），**禁止**完成 commit 后直接 `git push` + `gh pr create`。必须先跑对抗性 review，结论为 `APPROVE` 或 `CLEAR` 且无新增 blocker/major 问题后才能开 PR。
...
- ❌ commit 完成立刻 `git push` + 开 PR，把半成品扔给 reviewer（浪费人类时间；2026-05-25 add-execution-target-abstraction 提案就是教训：Codex 抓出 13 个 blocker/major，本应在开 PR 前修掉）
- ❌ 自己脑内"快速 review 了一下没问题"就开 PR —— 主 agent 的 review 受设计先入为主影响，必须交独立 subagent
- ❌ Review 抓出问题后 force-push 修，留下杂乱 commit 历史；正确做法是 commit 后**先 review** 再 push
