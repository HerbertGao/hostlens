...
- **[远程触发放大 backend 花费]** `run_schedule_now` 让远程 LLM 能引发 Hostlens 自身 backend 的 token 消费 → **缓解**：只能触发**已存在**的 schedule（远程 LLM 无法构造任意巡检）+ pipeline 既有 `agent.token_budget_*` 硬上限 + 较长 timeout 但有限。文档/`mcp_description` 明示该工具非免费。
