...
定义 Hostlens `hostlens mcp` Typer 子命令组（M7）：`hostlens mcp serve` 以 stdio transport 前台启动 MCP Server（装配真实 `ToolRegistry` + per-dispatch `ToolContext` 工厂）。`mcp` 为 optional-dependency，缺失时捕获 `ImportError` 以退出码 1 + `pip install "hostlens[mcp]"` 提示退出（禁裸 traceback）；`hostlens doctor` 新增 `checks.mcp`（`ok`/`missing`，非致命、不进 readiness 聚合），`--json` 与人类渲染均呈现。
