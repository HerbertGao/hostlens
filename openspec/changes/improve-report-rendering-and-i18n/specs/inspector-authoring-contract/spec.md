...
**(a)–(e) 是全部内置 inspector message 的最终目标契约,但 crosscheck 机审是分阶段启用的（避免范围矛盾）**:本提案只交付契约 + crosscheck 框架 + `systemd_failed_units` 旗舰样板,全量 72 个 inspector 的 message 改写是**分阶段多 PR 长尾**。若 crosscheck 上来就对**全量**断言「含中文」,71 个未改写的英文 message 会让它**一上线即全红、本提案归档即失败**。故 crosscheck 的 message-质量断言（中文 / 空指针 / 注入）**只施加在「已迁移 allowlist」**(初始 = `{systemd_failed_units}`,各域长尾 PR 改写完把成员逐个加入);allowlist 外的未迁移 inspector **暂不**受这些断言约束。**防漂移**由独立断言保证「每个内置 inspector 恰在 allowlist 或 backlog 之一、二者并集 == 全部内置」——新增 inspector 未分类即 crosscheck 失败,使「全量 vs 长尾」边界**显式可见**、不靠「碰巧没人加新 inspector」。
...
- **当** 遍历**已迁移 allowlist**(初始 = `{systemd_failed_units}`,随各域长尾 PR 改写完逐个加入)内 inspector 的 FindingRule `message`
- **那么** **必须**有测试断言:(d) 无 `see .* for details` 类空指针 pattern、(a) 含中文字符、(b/c) message 含 ≥1 `{field}` 注入**或** inspector 在 manifest 显式标注豁免(genuinely 无可变数据的纯标签类),且被注入的 `{field}` 必须在 `output_schema` 声明存在(防 `KeyError`)
- **且** crosscheck **不得**对 allowlist 外(未迁移)的 inspector 施加上述中文 / 注入断言——否则 71 个未改写英文 message 会让 crosscheck 一上线即全红、本提案归档即失败
- **且** crosscheck 是**静态**检查(**不**实例化 collector、**不**跑命令),其能力**仅限**上述「pattern + 中文 + 注入字段已声明 / 显式豁免」三项;它**不能**验证被注入的 `{field}` 在运行时是否真实存在、也**不能**验证注入值渲染是否干净(无 repr 泄漏)——那两项靠 collector 单测 + 真机 demo 兜,本契约不得宣称 crosscheck 覆盖运行时正确性
