...
- **命令必须经 `core/redact.py` `redact_text` best-effort 脱敏**后再渲染（与 audit 同源）。`redact_text` 覆盖 `key=value` / `Bearer` / JWT / `sk-` 形，**并覆盖已知客户端工具的 flag 形密钥**（`mysql -p<pw>` / `redis-cli -a <pw>` / `sshpass -p <pw>` / `curl -u user:<pw>` / `scheme://user:pw@host` URL userinfo 等，best-effort）。**已知残留**：**未知工具**的 flag 形密钥（如 `customcli -p<pw>`）仍不被覆盖、属残留，建议 plan 作者走 `exec` 的 `env` 注入。
...
#### 场景:命令脱敏后渲染
- **当** plan 的 `forward_cmd` 含 `redact_text` 可识别形态的敏感串（`key=value` / `Bearer` / JWT / `sk-`，或**已知工具 flag 形**如 `mysql -p<pw>`）
- **那么** runbook 中该串已脱敏；**未知工具**的 flag 形密钥仍属已知残留、不被覆盖
