...
request-key 算法目前内嵌在 `PlaybackBackend`（见 `llm-backend-protocol` spec）。recorder（写 canonical request）、lint（重复 key 检测）、playback（回放匹配）三处若各自复制算法，任一处对序列化参数 / 投影细节漂移就会造成「lint 认为无重复但 playback 实际冲突」或反向误报、或「recorder 写的 key 与 playback 读的不一致」。本提案**必须**把算法**逐字抽取**到单一无副作用 helper `hostlens.agent.cassette_key.request_key_for_payload(model, messages, tools_count) -> str`，由 `PlaybackBackend` / `RecordingBackend` / `cassette_lint.py` **共用**。

为避免在本 spec 引入与归档 `llm-backend-protocol` 竞争的「第二份精确算法描述」，本需求**不**重述序列化参数（`sort_keys` / `ensure_ascii` 等），而是把契约定义为「helper 必须与现有 `PlaybackBackend._request_key` 的输出**逐字节相等**」——即对任意 payload，helper 的 hash == 重构前 `PlaybackBackend` 既有算法的 hash。`PlaybackBackend` 改为调用该 helper 属**行为等价重构**，由 golden test 钉死这一相等性。
...
#### 场景:重构不改 playback hash（golden）
- **当** 对一组固定 payload 用 `request_key_for_payload` 计算 hash
- **那么** 结果必须等于 M2.1 `PlaybackBackend` 既有算法对同一 payload 的 hash（golden 值），证明重构行为等价
