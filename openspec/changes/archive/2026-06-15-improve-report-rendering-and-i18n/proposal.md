...
- **不**在本提案内原子完成全量 72 个 inspector message 改写。本提案交付:①模板重做 + ③中文根因叙述 + ②契约 & crosscheck 框架 + `linux.systemd.failed_units` 旗舰样板。其余 inspector 的 message 中文化 + 数据注入是**分阶段多 PR 长尾**,逐域(计算 / 内存 / 磁盘 / 网络 / 服务…)推进、crosscheck 作为防漂移门;不要求一次性合入。
...
- **代码**:telegram / lark 模板 + 新 Jinja filters(sev_label / conf_label / coverage / dedup / sort_sev / fmt_time);本提案内只改 `linux.systemd.failed_units` 旗舰样板(中文标签 + 注入干净串);diagnostician 系统提示(中文叙述);渲染时 dedup / sort、多 target 按 `finding.target_name` 分节(依赖提案 B)。其余 71 个 inspector YAML 的 `message` 重写在后续多 PR 长尾完成。
