...
定义 Hostlens 的 MCP Server 入口（M7，兑现「CLI + MCP Server」双交付的 MCP 一半）：基于官方 `mcp` SDK 的 stdio server，`build_server` 在构造时 eager 自检（fail-closed 投影），注册 `list_tools`（委托 `McpToolsAdapter.list_for_mcp()`）与 `call_tool`（委托 `dispatch`，统一捕获 `ToolPolicyViolation`/`KeyError`/`TypeError`/`ToolError` 为 `isError` 并脱敏，仅 `CancelledError` 放行），`run_stdio` 在 stdin EOF 时优雅退出。MCP Server 禁止持有或调用 `LLMBackend`（ADR-008）；本期 stdio-only，HTTP transport 与远程鉴权为 Non-Goal。
